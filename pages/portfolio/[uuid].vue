<template>
  <div
    ...
    <div class="h-full w-full flex flex-col md:flex-row justify-between md:container md:mx-auto default-screen-height">
      <div class="md:px-0 px-6 mt-5 md:flex md:justify-center md:w-1/2 md:flex-col">
        <div class="md:flex md:flex-col md:justify-end">

          <h1 class="text-white uppercase font-bold text-2xl md:text-4xl xl:text-6xl mb-5">{{
          ...
          <a class="text-accent lg:text-xl lg:mt-5" :href="asLink(data.data.website_url)" target="_blank"
             rel="noreferrer">Zobacz
            stronę</a></div>
      </div>
      <div
  ...
  <div class="bg-white text-black py-14 w-full md:px-0 px-6">
    <div class="container mx-auto">
      <div class="text-secondary flex flex-wrap md:justify-around">
        ...
        <div class="w-1/2 md:w-auto mt-3 md:mt-0"><p class="uppercase font-semibold text-xl lg:text-2xl">Technology</p>
          <div class="flex flex-wrap">
            <div v-for="item in data.data.technologies" class="ml-1">
              <img class="rounded-full w-[32px] h-[32px]" :src="`/skills/${item.technology}.png`"
                   :alt="item.technology">
            </div>
          </div>
        </div>
        <div class="w-1/2 md:w-auto mt-3 md:mt-0">
          ...
          <p class="font-normal lg:text-lg">{{ asText(data.data.seo_title) }}</p></div>
      </div>
      <img class="mx-auto mt-14 lg:px-44" :src="asImageSrc(data.data.responsive_device_mockup)"
...
const {
  data,
  refresh
} = await useAsyncData('portfolio.' + route.params.uuid, () => client.getByUID('portfolio', route.params.uuid, {lang: locale.value ?? 'pl'}))
